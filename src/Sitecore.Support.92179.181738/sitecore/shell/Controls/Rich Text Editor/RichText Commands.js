...
function scInsertSitecoreLink(sender, returnValue) {
    ...
    var linkElement = getURLField(document.getElementsByTagName("iframe"), "LinkURL");
  
    if (linkElement != null) {
        linkElement.value = returnValue.url;
        ...
        if (text == "" || text == null || ((text != null) && (text.length == 15) && (text.substring(2, 15).toLowerCase() == "<p>&nbsp;</p>"))) {
            var textElement = getURLField(document.getElementsByTagName("iframe"), "LinkText");
            if (textElement != null) {
                textElement.value = returnValue.text;
            }
        }
    }
}
