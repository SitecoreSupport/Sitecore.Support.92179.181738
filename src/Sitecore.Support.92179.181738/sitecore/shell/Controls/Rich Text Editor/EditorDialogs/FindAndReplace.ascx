	...
	Telerik.Web.UI.Widgets.FindAndReplace.prototype = {
		...
		isSelectionInContentArea: function(editor, findRange)
		{
			var utils = Telerik.Web.UI.Editor.Utils,
				...
				htmlArea = editor.get_textArea(),
				contentArea = isHtml ? htmlArea.parentNode : editor.get_contentArea();
		...
		getContentWindow: function() {
			...
			var htmlAreaDoc = htmlArea.ownerDocument;

			return isHtml ? (htmlAreaDoc.defaultView || htmlAreaDoc.parentWindow) : editor.get_contentWindow();
		},
		...
		replaceEngine: function (stringToFind, newString, backwards, replaceMode, wholeWord, caseSensitive, wrapContent)
		{
			...
			if (replaceMode == "replaceAll")
			{
				...
				if ($telerik.isIE)
				{
						...
						&& (this.isSelectionInContentArea(editor, rngReplace)))
					{
						...
						result = this._replace(editor, newString);
			...
			var found = false;
			var shouldWrapFind = (wrapContent && !this._wrapping);
			var seeker = this;
			var restartSearch = function() {
				seeker._wrapping = true;
				seeker.selectContentEdge(doc, !backwards);
				seeker.replaceEngine(stringToFind, newString, backwards, replaceMode, wholeWord, caseSensitive, wrapContent);
			};

			if (!contentWindow.find && $telerik.isIE)
			{
				var backwardsIndex = backwards ? -1 : 1;
				var rngFind = this.getSearchableRangeIE(doc, backwards);

				if ((found = rngFind.findText(stringToFind, backwardsIndex, flags))) {
					this._wrapping = false;
					rngFind.scrollIntoView();
					this.selectRange(rngFind);
				}
				else if(shouldWrapFind) {
			...
			else
			{
				...
				if ((found = contentWindow.find(stringToFind, caseSensitive, backwards, wrapContent, wholeWord, false, false)) || shouldWrapFind) {
					if(!found) {//find as failed, but we need to wrap around the search
						return restartSearch();
					}
					else if(!this.isSelectionInContentArea(editor)) {
						if(this._wrapping) {//second run over the content has failed
							found = this._wrapping = false;
							this.selectContentEdge(doc, !backwards);
						}
						else if(wrapContent) {
							return restartSearch();
						}
		...
		selectContentEdge: function(doc, toStart) {
			var editor = this._clientParameters.editor;
			var isHtml = this.isHtmlMode();
			var contentNode = isHtml ? editor.get_textArea() : editor.get_contentArea();

			if(isHtml) {
				contentNode.select();
				if(contentNode.selectionStart !== undefined) {
					contentNode.selectionStart = contentNode.selectionEnd = (toStart ? 0 : contentNode.value.length);
				}
				else {
				...
				var range = toDomRange(this.getRange(doc));
